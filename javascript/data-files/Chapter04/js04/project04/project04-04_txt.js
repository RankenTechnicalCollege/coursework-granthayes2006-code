...
function runTheRegister() {
  ...
  // Calculate change
  const changeValue = cashBox.value - billBox.value;

  try {
    // If cash < bill, throw an error
    if (changeValue < 0) {
      throw new Error("Cash amount doesn’t cover the bill");
    }

    // Display formatted change and breakdown
